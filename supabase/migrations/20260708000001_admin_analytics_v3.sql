...
--      and ratings with no time-based indexes.
--   2. rejection_by_domain runs SPLIT_PART over every moderation_queue row.
--
-- Fixes:
--   A. Add partial indexes on seen_urls(seen_at) and ratings(created_at)
--      covering the last 30 days so the active-user queries use index scans.
--   B. Increase statement_timeout to 30s to give headroom on cold caches.
