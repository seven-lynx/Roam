...
DECLARE
  ...
  v_collection_count BIGINT; v_follower_count BIGINT; v_following_count BIGINT; v_rate_count BIGINT := 0;
...
BEGIN
  ...
  SELECT COUNT(DISTINCT u.domain) INTO v_unique_domains FROM public.seen_urls su JOIN public.urls u ON u.id = su.url_id WHERE su.user_id = p_user_id;
  SELECT COUNT(DISTINCT u.category_id) INTO v_unique_cat_roam FROM public.seen_urls su JOIN public.urls u ON u.id = su.url_id WHERE su.user_id = p_user_id;
  ...
  SELECT p.streak_days, COALESCE(p.level,1), COALESCE(p.xp_total,0), EXTRACT(DAY FROM now() - p.created_at)::INT
  ...
  FOR v_badge IN SELECT b.* FROM public.badges b WHERE b.id NOT IN (SELECT ub.badge_id FROM public.user_badges ub WHERE ub.user_id = p_user_id) AND b.is_gift_only = FALSE AND b.category != 'milestone'
  LOOP
    ...
    CASE v_badge.slug
      ...
      WHEN 'pack-rat-bronze' THEN SELECT COALESCE(MAX(cic.cnt),0)::INT INTO v_progress FROM (SELECT COUNT(*) AS cnt FROM public.collection_items ci JOIN public.collections c ON c.id = ci.collection_id WHERE c.user_id = p_user_id GROUP BY c.id) cic; IF v_progress >= 10 THEN v_count := 1; END IF;
      WHEN 'pack-rat-silver' THEN SELECT COALESCE(MAX(cic.cnt),0)::INT INTO v_progress FROM (SELECT COUNT(*) AS cnt FROM public.collection_items ci JOIN public.collections c ON c.id = ci.collection_id WHERE c.user_id = p_user_id GROUP BY c.id) cic; IF v_progress >= 50 THEN v_count := 1; END IF;
      WHEN 'pack-rat-gold' THEN SELECT COALESCE(MAX(cic.cnt),0)::INT INTO v_progress FROM (SELECT COUNT(*) AS cnt FROM public.collection_items ci JOIN public.collections c ON c.id = ci.collection_id WHERE c.user_id = p_user_id GROUP BY c.id) cic; IF v_progress >= 200 THEN v_count := 1; END IF;
      ...
      WHEN 'rater-bronze' THEN v_progress := 0; v_count := 0;
      WHEN 'rater-silver' THEN v_progress := 0; v_count := 0;
      WHEN 'rater-gold' THEN v_progress := 0; v_count := 0;
      WHEN 'critic' THEN v_progress := 0; v_count := 0;
      WHEN 'omnivore' THEN v_progress := 0; v_count := 0;
    ...
    IF v_count > 0 THEN
      IF v_badge.parent_badge_slug IS NOT NULL THEN
        ...
        IF NOT EXISTS (SELECT 1 FROM public.user_badges ub2 WHERE ub2.user_id = p_user_id AND ub2.badge_id = v_parent_badge_id AND ub2.unlocked_at IS NOT NULL) THEN CONTINUE; END IF;
      END IF;
      ...
      SET progress_current = EXCLUDED.progress_current, unlocked_at = COALESCE(public.user_badges.unlocked_at, EXCLUDED.unlocked_at);
      IF FOUND THEN
        v_badge_xp_awarded := v_badge_xp_awarded + v_badge.xp_reward; v_new_count := v_new_count + 1;
        evaluate_badges.badge_id := v_badge.id; evaluate_badges.badge_slug := v_badge.slug; evaluate_badges.badge_name := v_badge.name; evaluate_badges.badge_description := v_badge.description; evaluate_badges.badge_icon := v_badge.icon; evaluate_badges.badge_category := v_badge.category; evaluate_badges.badge_tier := v_badge.tier; evaluate_badges.badge_xp_reward := v_badge.xp_reward;
        RETURN NEXT;
      END IF;
    ELSE
      INSERT INTO public.user_badges (user_id, badge_id, progress_current, unlocked_at)
  ...
  -- Milestone badges (all 18)
  FOR v_badge IN SELECT b.* FROM public.badges b WHERE b.category = 'milestone' AND b.is_gift_only = FALSE AND b.id NOT IN (SELECT mb.badge_id FROM public.user_badges mb WHERE mb.user_id = p_user_id)
  LOOP
    CASE v_badge.slug
      ...
      WHEN 'centurion-badges' THEN SELECT COUNT(*) INTO v_count FROM public.user_badges ub3 WHERE ub3.user_id = p_user_id AND ub3.unlocked_at IS NOT NULL; IF v_count >= 100 THEN v_count := 1; ELSE v_count := 0; END IF;
      WHEN 'master-roamer' THEN IF v_level >= 50 AND (SELECT COUNT(*) FROM public.user_badges ub4 WHERE ub4.user_id = p_user_id AND ub4.unlocked_at IS NOT NULL) >= 50 THEN v_count := 1; ELSE v_count := 0; END IF;
      WHEN 'grandmaster' THEN
        IF v_level >= 100 THEN
          SELECT COUNT(*) INTO v_count FROM public.user_badges ub5 JOIN public.badges gb ON gb.id = ub5.badge_id
          WHERE ub5.user_id = p_user_id AND ub5.unlocked_at IS NOT NULL AND gb.is_hidden = FALSE AND gb.is_gift_only = FALSE;
      ...
      WHEN 'demigod' THEN IF v_level >= 150 AND (SELECT COUNT(*) FROM public.user_badges ub6 WHERE ub6.user_id = p_user_id AND ub6.unlocked_at IS NOT NULL) >= 200 THEN v_count := 1; ELSE v_count := 0; END IF;
      ELSE CONTINUE;
    END CASE;
    IF v_count > 0 THEN
      INSERT INTO public.user_badges (user_id, badge_id, progress_current, unlocked_at)
      VALUES (p_user_id, v_badge.id, 0, now())
      ON CONFLICT (user_id, badge_id) DO UPDATE SET unlocked_at = COALESCE(public.user_badges.unlocked_at, EXCLUDED.unlocked_at);
      IF FOUND THEN
        v_badge_xp_awarded := v_badge_xp_awarded + v_badge.xp_reward; v_new_count := v_new_count + 1;
        evaluate_badges.badge_id := v_badge.id; evaluate_badges.badge_slug := v_badge.slug; evaluate_badges.badge_name := v_badge.name; evaluate_badges.badge_description := v_badge.description; evaluate_badges.badge_icon := v_badge.icon; evaluate_badges.badge_category := v_badge.category; evaluate_badges.badge_tier := v_badge.tier; evaluate_badges.badge_xp_reward := v_badge.xp_reward;
        RETURN NEXT;
