...
--   the planner prefers idx_urls_roam_candidates for the UNION ALL plan.
--
-- Fix:
...
--
--   2. ANALYZE key tables so the planner has fresh statistics for the
--      expression index (idx_urls_fallback_sort) when the fallback does run.
--
--   3. Bump statement_timeout to 35s (was still 25s in v12; our earlier
--      migration 20260505000000 re-created the function with 35s but we
--      cannot verify that applied cleanly, so we set it explicitly here).
-- =============================================================================
...
BEGIN
  ...
  ELSE
    ...
    -- unlikely unless the filtered pool is very small (e.g. niche subcategory).
    IF v_url_id IS NULL THEN
      SELECT c.id INTO v_url_id
      FROM (
        SELECT u.id,
...
-- Refresh planner statistics so the expression index (idx_urls_fallback_sort)
-- and the new domain-cooldown index get accurate row-count estimates.
ANALYZE public.urls;
ANALYZE public.seen_urls;
ANALYZE public.user_domain_cooldowns;
ANALYZE public.user_interest_scores;
