-- Fix evaluate_badges to handle missing url_ratings table
-- The table url_ratings was referenced in the evaluation function but never created.
-- This replaces evaluate_badges with a version that gracefully handles the missing table.
...
DECLARE
  ...
  v_streak_days INT; v_level INT; v_xp_total BIGINT; v_account_age_days INT; v_created_at TIMESTAMPTZ;
...
BEGIN
  ...
  -- url_ratings table may not exist; default to 0 if it doesn't
  BEGIN
    SELECT COUNT(*) INTO v_rate_count FROM public.url_ratings WHERE user_id = p_user_id;
  EXCEPTION WHEN undefined_table THEN v_rate_count := 0; END;
  BEGIN
  ...
  EXCEPTION WHEN undefined_table THEN v_unique_domains := 0; END;
  BEGIN
  ...
  EXCEPTION WHEN undefined_table THEN v_unique_cat_roam := 0; END;
  BEGIN
  ...
  EXCEPTION WHEN undefined_table THEN v_unique_cat_save := 0; END;
  SELECT p.streak_days, COALESCE(p.level,1), COALESCE(p.xp_total,0), p.created_at INTO v_streak_days, v_level, v_xp_total, v_created_at FROM public.profiles p WHERE p.id = p_user_id;
  ...
  EXCEPTION WHEN undefined_table THEN v_today_roam := 0; v_today_save := 0; END;
