...
BEGIN
  ...
  IF p_collection_id IS NOT NULL THEN
      ...
      SELECT u.id,
             (u.roam_score_static
               ...
               * CASE
                 ...
                 END
               AS eff_score
      FROM   urls u TABLESAMPLE BERNOULLI(15)
    ...
    -- Phase 2 fallback: wider scan, LIMIT 500 (v30: 50)
  ...
  ELSE
    ...
    -- Phase 2 fallback: wider scan, LIMIT 500 (v30: 100)
    IF v_url_id IS NULL THEN
      SELECT c.id INTO v_url_id
      FROM (
        SELECT u.id,
          ...
          SELECT u.id,
                 (u.roam_score_static
                   ...
                   * CASE
                     ...
                     END
                   AS eff_score
          FROM   urls u TABLESAMPLE BERNOULLI(30)
          INNER  JOIN collection_items ci ON ci.url_id = u.id
          WHERE  ci.collection_id = p_collection_id
            AND  u.approved       = TRUE
            AND  u.wilson_score   > -0.1
