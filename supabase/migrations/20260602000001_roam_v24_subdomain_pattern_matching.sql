-- =============================================================================
-- roam() v24 — subdomain pattern matching for exclude_domain and suppressions
-- =============================================================================
--
-- Problem:
--   Users downvoting different subdomains (e.g., artist1.bandcamp.com,
--   artist2.bandcamp.com) didn't trigger domain suppression because suppression
--   requires 2+ downvotes on the SAME domain. Subdomains were treated as separate
--   domains, so each needed 2+ downvotes independently.
--
-- Fix:
--   1. Exclude domain filter now matches parent domains via LIKE pattern
--      p_exclude_domain = "bandcamp.com" now excludes artist1.bandcamp.com,
--      artist2.bandcamp.com, etc. (extension extracts root domain)
--   2. Suppression filter now matches parent domains in user_suppressed_domains
--      Suppressing "bandcamp.com" now covers all .bandcamp.com subdomains
--   3. Both changes use pattern matching: u.domain LIKE ('%.' || suppressed_domain)
--      consistent with existing paywall domain logic
--
-- Builds on v23 domain cooldown enhancements. Suppression logic now provides
-- root-level domain blocking while cooldowns remain at subdomain-level granularity.
-- =============================================================================
  ...
  IF p_collection_id IS NOT NULL THEN
    ...
    FROM (
      ...
      WHERE  ci.collection_id = p_collection_id
        ...
        -- Exclude domain (with subdomain matching): p_exclude_domain covers root domain and all subdomains
        AND  (p_exclude_domain IS NULL OR (u.domain IS DISTINCT FROM p_exclude_domain AND u.domain NOT LIKE ('%.' || p_exclude_domain)))
        -- Suppressed domains (with subdomain matching): both exact match and parent domain match
        AND  NOT EXISTS (
    ...
    IF v_url_id IS NULL THEN
      ...
      FROM (
        ...
        WHERE  ci.collection_id = p_collection_id
          ...
          AND  u.wilson_score   > -0.1
          AND  u.language       = ANY(v_langs)
          -- Exclude domain (with subdomain matching)
          AND  (p_exclude_domain IS NULL OR (u.domain IS DISTINCT FROM p_exclude_domain AND u.domain NOT LIKE ('%.' || p_exclude_domain)))
          -- Suppressed domains (with subdomain matching)
          AND  NOT EXISTS (
                 SELECT 1 FROM user_suppressed_domains usd
                 WHERE usd.user_id = p_user_id
                   AND usd.suppressed_until > NOW()
