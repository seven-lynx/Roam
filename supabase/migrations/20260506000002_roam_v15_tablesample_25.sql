...
--
-- Problem:
--   With correlated NOT EXISTS subqueries now replaced by pre-loaded arrays
--   (v14), Phase 1 is significantly cheaper per-row. However BERNOULLI(10)
--   still only samples ~315k of 3.15M rows before applying all WHERE filters
--   (language, category, seen, cooled, suppressed). For users with niche
--   category preferences and a meaningful seen-URLs history, the post-filter
...
--
--   With v14's pre-loaded arrays reducing per-row filter cost by ~70%, the
--   wider sample is still well within the 35s statement_timeout budget.
-- =============================================================================
