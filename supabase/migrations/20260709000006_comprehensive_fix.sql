-- Final comprehensive fix for evaluate_badges:
-- 1. Fix column ambiguity (badge_id vs id in subqueries)
-- 2. Restore master-roamer and grandmaster milestone badges
-- 3. Keep all previous fixes (ON CONFLICT DO UPDATE, missing table handlers, etc.)
...
BEGIN
  ...
  -- Stats (wrap each in exception handler)
  ...
  FOR v_badge IN
    ...
    WHERE b.is_gift_only = FALSE AND b.category != 'milestone'
    AND NOT EXISTS (
  ...
  LOOP
    ...
    IF v_count > 0 THEN
      IF v_badge.parent_badge_slug IS NOT NULL THEN
        ...
        IF NOT EXISTS (SELECT 1 FROM public.user_badges ub WHERE ub.user_id = p_user_id AND ub.badge_id = v_parent_badge_id AND ub.unlocked_at IS NOT NULL) THEN CONTINUE; END IF;
      END IF;
      ...
      DO UPDATE SET progress_current = EXCLUDED.progress_current, unlocked_at = COALESCE(user_badges.unlocked_at, now());

      SELECT ub2.unlocked_at INTO v_existing_row FROM public.user_badges ub2 WHERE ub2.user_id = p_user_id AND ub2.badge_id = v_badge.id;
  ...
  LOOP
    CASE v_badge.slug
      ...
      WHEN 'grandmaster' THEN
        IF v_level >= 100 THEN
          SELECT COUNT(*) INTO v_count FROM public.user_badges ub5 JOIN public.badges b3 ON b3.id = ub5.badge_id WHERE ub5.user_id = p_user_id AND ub5.unlocked_at IS NOT NULL AND b3.is_hidden = FALSE AND b3.is_gift_only = FALSE;
          IF v_count >= (SELECT COUNT(*) FROM public.badges b4 WHERE b4.is_hidden = FALSE AND b4.is_gift_only = FALSE AND b4.category != 'milestone') THEN v_count := 1; ELSE v_count := 0; END IF;
    ...
    IF v_count > 0 THEN
      ...
      ON CONFLICT (user_id, badge_id) DO UPDATE SET unlocked_at = COALESCE(user_badges.unlocked_at, now());

      SELECT ub6.unlocked_at INTO v_existing_row FROM public.user_badges ub6 WHERE ub6.user_id = p_user_id AND ub6.badge_id = v_badge.id;
      IF v_existing_row.unlocked_at IS NOT NULL THEN
        v_badge_xp_awarded := v_badge_xp_awarded + v_badge.xp_reward; v_new_count := v_new_count + 1;
        INSERT INTO public.notifications (user_id, type, title, body, data)
        VALUES (p_user_id, 'badge_unlocked', v_badge.icon || ' Badge Unlocked: ' || v_badge.name, v_badge.description, jsonb_build_object('badge_slug', v_badge.slug, 'badge_icon', v_badge.icon, 'badge_name', v_badge.name, 'xp_reward', v_badge.xp_reward, 'url', v_profile_url));
