-- Fix: evaluate_badges references su.seen_url_id but the column is su.url_id
-- Apply this single statement to correct all references in the function.

-- Update the function body by replacing seen_url_id with url_id everywhere
...
BEGIN
  ...
  -- Collect user stats (using correct column name: url_id, not seen_url_id)
  ...
  FOR v_badge IN
    ...
    WHERE is_gift_only = FALSE AND category != 'milestone'
    AND NOT EXISTS (
  ...
  -- Milestone badges
  FOR v_badge IN
    SELECT * FROM public.badges WHERE category = 'milestone' AND is_gift_only = FALSE
    AND NOT EXISTS (SELECT 1 FROM public.user_badges ub WHERE ub.user_id = p_user_id AND ub.badge_id = badges.id AND ub.unlocked_at IS NOT NULL)
  LOOP
    CASE v_badge.slug
  ...
  IF v_level > v_prev_level THEN
    INSERT INTO public.notifications (user_id, type, title, body, data)
    VALUES (p_user_id, 'level_up', chr(127881) || ' Level Up! You''re now Level ' || v_level, 'Keep roaming to earn more badges and XP!', jsonb_build_object('level', v_level, 'rank', '', 'url', v_profile_url));
  END IF;
END;
$$;
