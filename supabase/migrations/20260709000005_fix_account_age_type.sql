-- Fix: v_account_age_days was declared INT but receiving p.created_at (timestamptz)
-- via SELECT INTO, causing "invalid input syntax for type integer" errors.
-- Add a v_created_at variable and compute v_account_age_days from it.
...
BEGIN
  ...
  -- Profile data - use v_created_at (timestamptz) then compute v_account_age_days (INT)
