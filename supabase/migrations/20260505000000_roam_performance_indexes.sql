-- =============================================================================
-- roam() performance indexes
-- =============================================================================
--
-- Problem: roam() SQL function frequently hits its 25s statement_timeout,
...
--    (user_id, cooldown_until) but the query filters on (user_id, domain,
--    cooldown_until > NOW()). PostgreSQL must scan all rows per user.
...
--
-- Fix:
--   a. Expression index on urls for the fallback sort expression.
--   b. Replace domain cooldown index with (user_id, domain) partial index
--      filtered to active cooldowns.
--   c. Add (user_id, calibrated_weight DESC) index for deep-dive sort.
--   d. Bump statement_timeout from 25s to 35s for headroom.
-- =============================================================================

-- a. Expression index for fallback ORDER BY (wilson_score + 0.3 * seeder_score)
CREATE INDEX IF NOT EXISTS idx_urls_fallback_sort
  ON public.urls ((wilson_score + 0.3 * seeder_score) DESC)
  WHERE approved = TRUE;

-- b. Replace the domain cooldown index so the correlated subquery
--    WHERE user_id = X AND domain = Y AND cooldown_until > NOW()
--    becomes an index-only lookup.
DROP INDEX IF EXISTS idx_user_domain_cooldowns;
CREATE INDEX IF NOT EXISTS idx_user_domain_cooldowns_lookup
  ON public.user_domain_cooldowns (user_id, domain);

-- c. Index for deep-dive subcategory selection:
--    ORDER BY calibrated_weight DESC LIMIT 3 WHERE user_id = X AND calibrated_weight > 1.0
CREATE INDEX IF NOT EXISTS idx_user_interest_scores_weight
  ON public.user_interest_scores (user_id, calibrated_weight DESC)
  WHERE calibrated_weight > 1.0;

-- d. Bump statement_timeout from 25s to 35s.
