...
-- and Phase 2 (full-scan fallback), causing two problems:
--   1. Dead URLs (inactive = true) enter the candidate pool and get served.
--   2. TABLESAMPLE and Phase 2 scan the full 3.2M approved-URL set instead of
--      the 1.7M active subset, nearly doubling I/O and causing intermittent
--      25-second edge function timeouts (DataFileRead stalls).
--
-- Fixes:
--   - Add `u.inactive = FALSE` to every WHERE clause in both modes.
--   - Create idx_urls_roam_score_active so Phase 2 ORDER BY can use a 1.7M-entry
--     index instead of the existing 3.2M-entry idx_urls_roam_score_static.

-- Phase-2 fallback index: ordered by roam_score_static over ACTIVE URLs only.
CREATE INDEX IF NOT EXISTS idx_urls_roam_score_active
