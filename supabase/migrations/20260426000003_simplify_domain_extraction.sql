-- Fix domain extraction - use split_part instead of complex regex
-- POSIX regex in PostgreSQL doesn't support non-capturing groups
...
BEGIN
  ...
  -- Extract domain from p_current_domain if provided (remove www prefix)
  v_extracted_domain := CASE
    WHEN p_current_domain IS NULL THEN NULL
    WHEN p_current_domain LIKE 'www.%' THEN lower(substring(p_current_domain FROM 5))
    ELSE lower(p_current_domain)
  END;
  ...
  IF p_collection_id IS NOT NULL THEN
    ...
    WHERE ci.collection_id = p_collection_id
      ...
      AND (
        -- If p_current_domain is provided, exclude URLs from that domain
        v_extracted_domain IS NULL
        OR lower(regexp_replace(split_part(split_part(u.url, '://', 2), '/', 1), '^www\.', '')) != v_extracted_domain
      )
  ...
  ELSE
    ...
    WHERE u.approved = TRUE
      AND (
        ...
        OR
        -- Case 2: URL has NULL subcategory (unassigned)
