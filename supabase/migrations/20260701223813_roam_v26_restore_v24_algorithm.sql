...
-- DESC LIMIT 1, no seen_urls recording, no domain cooldown, no interest weighting
-- and no randomization. Result: roam returned the same handful of top-scored URLs
-- forever (users saw "the same 2 URLs").
--
-- This restores v24 verbatim (TABLESAMPLE variety, weighted-random ordering,
-- interest scoring, subdomain-aware exclusion/suppression, domain cooldowns, and
-- the INSERT INTO seen_urls that gives roam its memory) and adds the
-- p_exclude_domains TEXT[] parameter the edge function sends, with subdomain-aware
-- matching across the whole exclusion array.
-- =============================================================================
...
DECLARE
  ...
  v_excluded            TEXT[];  -- merged p_exclude_domain + p_exclude_domains
BEGIN
