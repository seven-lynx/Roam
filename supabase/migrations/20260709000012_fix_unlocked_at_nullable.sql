-- Fix: unlocked_at NOT NULL DEFAULT now() means progress rows (which omit unlocked_at)
-- get DEFAULT now() instead of NULL, causing every badge to appear "earned."
--
-- Solution: Make unlocked_at nullable. Progress rows use NULL. Earned rows use now().
-- Then fix evaluate_badges to explicitly set unlocked_at=NULL for progress rows.

-- 1. Remove bad backfill data again
...
-- 2. Make unlocked_at nullable (so progress rows can be NULL)
ALTER TABLE public.user_badges
  ALTER COLUMN unlocked_at DROP NOT NULL,
  ALTER COLUMN unlocked_at DROP DEFAULT;

-- 3. Replace evaluate_badges with final correct version
--    Progress rows: INSERT with unlocked_at = NULL
--    Earned rows:   INSERT with unlocked_at = now()
--    ON CONFLICT:   UPDATE unlocked_at = COALESCE(old, EXCLUDED)
...
BEGIN
  ...
  LOOP
    ...
    CASE v_badge.slug
      ...
      WHEN 'speed-collector' THEN v_progress := LEAST(v_today_save, 10); IF v_today_save >= 10 THEN v_count := 1; END IF;
      WHEN 'mega-collector' THEN v_progress := LEAST(v_today_save, 50); IF v_today_save >= 50 THEN v_count := 1; END IF;
      ...
      -- Manually-granted only (no automatic criteria): first-share, easter-egg, friendly-face,
      -- citizen-journalist, omnivore, favorited-*
      ELSE
        v_count := 0;
    END CASE;
  ...
  LOOP
    CASE v_badge.slug
      ...
      WHEN 'grandmaster' THEN IF v_level >= 100 THEN SELECT COUNT(*) INTO v_count FROM public.user_badges ub JOIN public.badges b ON b.id = ub.badge_id WHERE ub.user_id = p_user_id AND ub.unlocked_at IS NOT NULL AND b.is_hidden = FALSE AND b.is_gift_only = FALSE; IF v_count >= (SELECT COUNT(*) FROM public.badges WHERE is_hidden = FALSE AND is_gift_only = FALSE AND category != 'milestone') THEN v_count := 1; ELSE v_count := 0; END IF; ELSE v_count := 0; END IF;
      ELSE CONTINUE;
    END CASE;
