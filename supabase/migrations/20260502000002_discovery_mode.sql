-- =============================================================================
-- Explore/Exploit Discovery Mode Toggle (Task 11.30)
-- =============================================================================
--
-- Adds a `discovery_mode` preference to user_settings with two values:
--
--   'discovery' (default)
--     Current behaviour: calibrated weights + 12% adjacent category serving.
--     Mix of known interests and serendipitous edges.
--
--   'deep_dive'
--     Focused mode: serve only from the user's top-3 subcategories by
--     calibrated_weight. Adjacent serving disabled. Best for users who
...
BEGIN
  ...
  THEN
    ...
    FROM (
      ...
      WHERE  user_id = p_user_id
        AND  calibrated_weight > 1.0   -- only above-median subcategories
      ORDER  BY calibrated_weight DESC
      LIMIT  3
    ) t;
    -- Fall back to full allowed list if insufficient data
  ...
  -- Effective subcategory: explicit pin > adjacent > deep_dive list
  ...
  ELSE
    ...
    FROM (
      ...
      WHERE  u.approved = TRUE
        ...
        -- Deep dive: restrict to top-3 calibrated subcategories
        AND  (
               v_deep_dive_subcats IS NULL
               OR u.subcategory_id = ANY(v_deep_dive_subcats)
             )
        -- User category prefs (skipped when pinned/adjacent/deep-dive narrows)
        AND  (
