-- Fix two issues preventing evaluate_badges from working:
-- 1. Progress INSERTs pass NULL for unlocked_at which violates NOT NULL. Omit the column.
-- 2. Drop the broken pg_net webhook trigger (users should configure via Supabase Dashboard).

-- Drop the broken pg_net trigger first
DROP TRIGGER IF EXISTS on_notification_insert ON public.notifications;
DROP FUNCTION IF EXISTS public.notify_webhook_trigger();

-- Recreate evaluate_badges with the NULL fix
...
BEGIN
  ...
  LOOP
    ...
    IF v_count > 0 THEN
      -- Award: insert with now() or update existing to set unlocked_at
      ...
      RETURN NEXT;
    ELSE
      -- Progress: omit unlocked_at so DEFAULT now() kicks in
