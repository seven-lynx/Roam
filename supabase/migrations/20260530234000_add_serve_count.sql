-- Add serve_count to urls and increment it each time a URL is served by roam().
-- This enables hygiene queries like: urls served many times but never voted on.
...
BEGIN
  ...
  IF v_url_id IS NOT NULL THEN
    ...
    UPDATE urls SET serve_count = serve_count + 1 WHERE id = v_url_id;
