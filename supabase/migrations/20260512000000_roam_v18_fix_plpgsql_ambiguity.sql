...
-- while the function RETURNS TABLE also defines `subcategory_id` as an output
-- variable. In PL/pgSQL that can become ambiguous and fail at runtime with:
--   ERROR: column reference "subcategory_id" is ambiguous
--
-- This migration hardens roam() by:
--   1) Adding `#variable_conflict use_column` so SQL column references win
--      over same-named output variables.
--   2) Explicitly aliasing the user_interest_scores load query (uis.*) to
--      make intent unambiguous.
--
-- No logic changes from v17.
-- =============================================================================
