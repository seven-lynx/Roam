-- 1. Remove all badges awarded during the faulty backfill (last 2 hours)
--    Keep progress rows intact since they're harmless.
DELETE FROM public.user_badges
WHERE unlocked_at IS NOT NULL
  AND unlocked_at > now() - INTERVAL '2 hours';

-- Also remove xp_log entries from the bad backfill
DELETE FROM public.xp_log
WHERE action = 'badge_rewards'
  AND created_at > now() - INTERVAL '2 hours';

-- Reset badge counts and recalculate from actual unlocked badges
UPDATE public.profiles p
SET badge_count = (
  SELECT COUNT(*) FROM public.user_badges ub
  WHERE ub.user_id = p.id AND ub.unlocked_at IS NOT NULL
);

-- 2. Replace evaluate_badges with a version that uses CLEAN, proven logic
--    based on the original 20260613000001 version but with all bugs fixed:
--    - OUT params renamed to avoid column collisions
--    - ON CONFLICT DO UPDATE SET (the critical fix)
--    - su.url_id (correct column name)
--    - v_account_age_days uses v_created_at intermediate
--    - progress INSERTs omit unlocked_at column
...
BEGIN
  ...
  -- Stats with graceful fallback for missing tables
  ...
  -- Today's counts (from current session, not cumulative)
  ...
  -- =========================================================================
  -- Standard badges -- ONLY evaluate badges that the user hasn't already earned.
  -- The ELIGIBILITY CHECK ensures each badge only unlocks when its SPECIFIC
  -- criteria are met. Parent badges must be earned before children.
  -- =========================================================================
  FOR v_badge IN
  ...
  LOOP
    ...
    CASE v_badge.slug
      ...
      WHEN 'completionist' THEN v_progress := v_unique_cat_save::INT; IF v_unique_cat_save >= v_total_categories AND v_total_categories > 0 THEN v_count := 1; END IF;
      ...
      WHEN 'pack-rat-bronze' THEN BEGIN SELECT COALESCE(MAX(cnt),0)::INT INTO v_progress FROM (SELECT COUNT(*) AS cnt FROM public.collection_items ci JOIN public.collections c ON c.id = ci.collection_id WHERE c.user_id = p_user_id GROUP BY c.id) sub; EXCEPTION WHEN undefined_table THEN v_progress := 0; END; IF v_progress >= 10 THEN v_count := 1; END IF;
      WHEN 'pack-rat-silver' THEN BEGIN SELECT COALESCE(MAX(cnt),0)::INT INTO v_progress FROM (SELECT COUNT(*) AS cnt FROM public.collection_items ci JOIN public.collections c ON c.id = ci.collection_id WHERE c.user_id = p_user_id GROUP BY c.id) sub; EXCEPTION WHEN undefined_table THEN v_progress := 0; END; IF v_progress >= 50 THEN v_count := 1; END IF;
      ...
      -- Badges intentionally left as manually-granted (v_count stays 0):
      -- first-share, easter-egg, omnivore, friendly-face, citizen-journalist, favorited-*
      ELSE
        v_count := 0;
    END CASE;

    IF v_count > 0 THEN
      -- Check parent badge requirement
      IF v_badge.parent_badge_slug IS NOT NULL THEN
        IF NOT EXISTS (
          SELECT 1 FROM public.user_badges pub
          JOIN public.badges pb ON pb.id = pub.badge_id
          WHERE pub.user_id = p_user_id
            AND pb.slug = v_badge.parent_badge_slug
            AND pub.unlocked_at IS NOT NULL
        ) THEN
          CONTINUE;
        END IF;
      END IF;

      -- CRITICAL: award badge with unlocked_at=now(), updating progress rows that existed
      ...
      out_badge_slug := v_badge.slug;
      out_badge_name := v_badge.name;
      out_badge_description := v_badge.description;
      out_badge_icon := v_badge.icon;
      out_badge_category := v_badge.category;
      out_badge_tier := v_badge.tier;
      out_badge_xp_reward := v_badge.xp_reward;
      RETURN NEXT;
    ELSE
      -- Progress-only: omit unlocked_at so DEFAULT picks now()
  ...
  FOR v_badge IN
    ...
    WHERE b.category = 'milestone' AND b.is_gift_only = FALSE
    AND NOT EXISTS (
      SELECT 1 FROM public.user_badges ub
      WHERE ub.user_id = p_user_id AND ub.badge_id = b.id AND ub.unlocked_at IS NOT NULL
    )
  LOOP
    CASE v_badge.slug
      ...
      WHEN 'centurion-badges' THEN
        SELECT COUNT(*) INTO v_count FROM public.user_badges WHERE user_id = p_user_id AND unlocked_at IS NOT NULL;
        IF v_count >= 100 THEN v_count := 1; ELSE v_count := 0; END IF;
      WHEN 'master-roamer' THEN
        IF v_level >= 50 AND (SELECT COUNT(*) FROM public.user_badges WHERE user_id = p_user_id AND unlocked_at IS NOT NULL) >= 50 THEN
          v_count := 1;
        ELSE v_count := 0; END IF;
      WHEN 'grandmaster' THEN
        IF v_level >= 100 THEN
          SELECT COUNT(*) INTO v_count FROM public.user_badges ub
          JOIN public.badges b ON b.id = ub.badge_id
          WHERE ub.user_id = p_user_id AND ub.unlocked_at IS NOT NULL
            AND b.is_hidden = FALSE AND b.is_gift_only = FALSE;
          IF v_count >= (SELECT COUNT(*) FROM public.badges WHERE is_hidden = FALSE AND is_gift_only = FALSE AND category != 'milestone') THEN
            v_count := 1;
          ELSE v_count := 0; END IF;
        ELSE v_count := 0; END IF;
      ELSE CONTINUE;
    ...
    IF v_count > 0 THEN
      ...
      ON CONFLICT (user_id, badge_id)
      DO UPDATE SET unlocked_at = COALESCE(public.user_badges.unlocked_at, now());
  ...
  -- =========================================================================
  -- Award XP for newly earned badges, then recalculate level
  -- =========================================================================
