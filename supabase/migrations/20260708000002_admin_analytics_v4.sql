...
--      even with indexes, DISTINCT UNION across 3 time windows is too costly.
--   2. rejection_by_domain runs SPLIT_PART + GROUP BY over all moderation_queue.
--
-- Fix:
...
SELECT
  (
    SELECT COUNT(DISTINCT user_id) FROM (
      SELECT user_id FROM public.seen_urls WHERE seen_at >= NOW() - INTERVAL '24 hours'
      UNION
      SELECT user_id FROM public.ratings WHERE created_at >= NOW() - INTERVAL '24 hours'
    ) sub
  )::int AS dau,
  (
    SELECT COUNT(DISTINCT user_id) FROM (
      SELECT user_id FROM public.seen_urls WHERE seen_at >= NOW() - INTERVAL '7 days'
      UNION
      SELECT user_id FROM public.ratings WHERE created_at >= NOW() - INTERVAL '7 days'
    ) sub
  )::int AS wau,
  (
    SELECT COUNT(DISTINCT user_id) FROM (
      SELECT user_id FROM public.seen_urls WHERE seen_at >= NOW() - INTERVAL '30 days'
      UNION
      SELECT user_id FROM public.ratings WHERE created_at >= NOW() - INTERVAL '30 days'
    ) sub
  )::int AS mau;
...
SELECT
  ...
  ROUND(
    (COUNT(*) FILTER (WHERE status = 'rejected')::numeric / COUNT(*)::numeric * 100),
    1
...
BEGIN
  ...
  FROM (
    SELECT domain, total, rejected, rejection_pct
    FROM public.mv_admin_rejection_by_domain
    ORDER BY rejection_pct DESC
  ) row;
...
SELECT cron.schedule(
  ...
  '15 * * * *',
  'REFRESH MATERIALIZED VIEW public.mv_admin_active_users'
);

SELECT cron.schedule(
  'refresh-mv-admin-rejection-by-domain',
  '20 * * * *',
  'REFRESH MATERIALIZED VIEW CONCURRENTLY public.mv_admin_rejection_by_domain'
);