...
--   categories using a new `v_allowed_cat_ids` array.
--
...
CREATE INDEX IF NOT EXISTS idx_user_category_scores
  ON public.user_category_scores (user_id, category_id);

ALTER TABLE public.user_category_scores ENABLE ROW LEVEL SECURITY;
CREATE POLICY "category_scores: users can read own"
...
BEGIN
  IF TG_OP = 'INSERT' THEN
    SELECT subcategory_id, category_id INTO v_subcat_id, v_cat_id
  ...
  -- Category-level scores (always, as long as URL has a category)
  IF v_cat_id IS NOT NULL THEN
    INSERT INTO user_category_scores
      (user_id, category_id, upvote_count, downvote_count, calibrated_weight, last_updated)
    VALUES (
      v_uid, v_cat_id,
      GREATEST(0, v_up_delta),
      GREATEST(0, v_down_delta),
    ...
    ON CONFLICT (user_id, category_id) DO UPDATE SET
      upvote_count   = GREATEST(0, user_category_scores.upvote_count   + v_up_delta),
      downvote_count = GREATEST(0, user_category_scores.downvote_count + v_down_delta),
      calibrated_weight = (
        CASE
          WHEN (GREATEST(0, user_category_scores.upvote_count   + v_up_delta)
              + GREATEST(0, user_category_scores.downvote_count + v_down_delta)) = 0
          THEN 1.0
          ELSE
            GREATEST(0, user_category_scores.upvote_count + v_up_delta)::FLOAT
            / (GREATEST(0, user_category_scores.upvote_count   + v_up_delta)
             + GREATEST(0, user_category_scores.downvote_count + v_down_delta))::FLOAT
            / 0.5
        END
      ),
      last_updated = NOW();
...
SELECT
  r.user_id,
  u.category_id,
  SUM(CASE WHEN r.value =  1 THEN 1 ELSE 0 END)::INTEGER,
  SUM(CASE WHEN r.value = -1 THEN 1 ELSE 0 END)::INTEGER,
  CASE
    WHEN SUM(CASE WHEN r.value =  1 THEN 1 ELSE 0 END)
       + SUM(CASE WHEN r.value = -1 THEN 1 ELSE 0 END) = 0
    THEN 1.0
    ELSE
      SUM(CASE WHEN r.value =  1 THEN 1 ELSE 0 END)::FLOAT
      / (SUM(CASE WHEN r.value =  1 THEN 1 ELSE 0 END)
       + SUM(CASE WHEN r.value = -1 THEN 1 ELSE 0 END))::FLOAT
      / 0.5
  END
FROM   ratings r
...
GROUP  BY r.user_id, u.category_id
ON CONFLICT (user_id, category_id) DO UPDATE
  SET upvote_count      = EXCLUDED.upvote_count,
