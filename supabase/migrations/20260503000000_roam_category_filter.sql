-- =============================================================================
-- Add p_category_id filter to roam() (Task: category-level roaming)
-- =============================================================================
--
-- Most URLs have category_id set but no subcategory_id. The existing
-- p_subcategory_id filter was therefore useless for category-level pins from
-- the Android app and extension. This migration adds p_category_id so callers
...
BEGIN
  ...
  -- Effective subcategory: explicit subcategory pin > adjacent serving
  ...
  ELSE
    ...
    FROM (
      ...
      WHERE  u.approved = TRUE
        ...
        -- User category prefs (bypassed when any pin is active)
        AND  (
