...
--
-- Problem:
--   Migration 20260503000001 added p_category_id + category_id return column
--   to the 5-param function.  Migration 20260503000005 (v11) introduced the
...
BEGIN
  ...
  ELSE
    ...
    FROM (
      ...
      WHERE  u.approved     = TRUE
        ...
        AND  u.language     = ANY(v_langs)
        AND  (p_exclude_domain IS NULL OR u.domain IS DISTINCT FROM p_exclude_domain)
        -- category pin (extension roamCategory / Android category roam)
        ...
        -- user-preference gate: bypass when a caller has pinned a specific scope
        AND  (
      ...
      -- Fallback full-scan (no TABLESAMPLE) ordered by score to ensure pool
      -- coverage when BERNOULLI(10) returns too few rows.
      SELECT fb.id, fb.eff_score
      FROM (
        SELECT u.id,
               (u.wilson_score
  ...
  IF v_url_id IS NOT NULL THEN
    ...
    -- Domain cooldown only in discovery mode (not collection mode, not deep_dive)
  ...
  -- Return the full row including category_id so callers don't need a
  -- second lookup to display the category badge.
  RETURN QUERY
