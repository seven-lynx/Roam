-- roam v25 (20260617000002_roam_v25_apply.sql) dropped DEFAULT NULL from the
-- optional parameters, leaving only p_exclude_domains defaulted. PostgREST could
-- therefore not resolve roam(p_user_id) for an unfiltered discovery call:
--   ERROR: 42883: function public.roam(p_user_id => uuid) does not exist
-- which the edge function returned as a 500 and the Android app mislabeled as
-- "Session expired." Restore trailing DEFAULT NULL on every optional parameter so
-- the edge function can invoke roam() with any subset of named arguments.
DROP FUNCTION IF EXISTS public.roam(UUID, UUID, TEXT, UUID, UUID, TEXT[]);
