-- Fix broken column reference in roam() and evaluate_badges().
--
...
-- "Session expired." Discovery was fully broken and no badge that counts roamed
-- URLs could ever evaluate.
--
...
BEGIN
  ...
  FROM public.seen_urls su JOIN public.urls u ON u.id = su.url_id WHERE su.user_id = p_user_id;
