...
DROP FUNCTION IF EXISTS public.url_effective_score(
  ...
  p_score_weights       DOUBLE PRECISION[],
  p_recent_subcats      UUID[]
);

CREATE FUNCTION public.url_effective_score(
  p_url_id              UUID,
  p_score_subcats       UUID[],
  p_score_weights       DOUBLE PRECISION[],
  p_recent_subcats      UUID[]
...
DECLARE
  ...
  v_subcat_id        UUID;
  v_interest_weight  DOUBLE PRECISION;
  v_recency_decay    DOUBLE PRECISION;
  v_explore_bonus    DOUBLE PRECISION;
  v_days_old         DOUBLE PRECISION;
  v_rotate_penalty   DOUBLE PRECISION;
  v_cold_start_bonus DOUBLE PRECISION;
  v_serve_count      INTEGER;
  v_upvote_count     INTEGER;
  v_downvote_count   INTEGER;
  v_published_at     TIMESTAMPTZ;
  v_roam_score       DOUBLE PRECISION;
BEGIN
  SELECT
    u.roam_score_static,
    u.subcategory_id,
    ...
    COALESCE(u.serve_count, 0)
  INTO v_roam_score, v_subcat_id, v_published_at,
       v_upvote_count, v_downvote_count, v_serve_count
  FROM urls u
  WHERE u.id = p_url_id;

  IF v_roam_score IS NULL THEN
    RETURN 0;
  END IF;

  -- Cold start bonus: no votes yet = slight boost
  v_cold_start_bonus := CASE WHEN (v_upvote_count + v_downvote_count) = 0 THEN 0.15 ELSE 0 END;

  -- Exploration bonus: low-serve-count URLs get boosted
  v_explore_bonus := CASE
    WHEN v_serve_count = 0 THEN 0.25
  ...
  -- Recency decay: older content gets a gentle penalty
  IF v_published_at IS NULL THEN
    v_recency_decay := 0.7;
  ELSE
    v_days_old := EXTRACT(EPOCH FROM (NOW() - v_published_at)) / 86400.0;
    v_recency_decay := GREATEST(EXP(-0.0003 * GREATEST(v_days_old, 0)), 0.2);
  END IF;

  -- Subcategory rotation: penalize recently-served subcategories (v30)
  v_rotate_penalty := 1.0;
  IF p_recent_subcats IS NOT NULL AND v_subcat_id = ANY(p_recent_subcats) THEN
    v_rotate_penalty := 0.3;
  END IF;

  v_score := (v_roam_score + v_cold_start_bonus + v_explore_bonus)
          * v_interest_weight
          * v_recency_decay
          * v_rotate_penalty;
...
DECLARE
  ...
  v_recent_subcats      UUID[];   -- v30: last 3 subcategories served
BEGIN
  ...
  -- v30: track last 3 subcategories served (for rotation/penalty)
  ...
  IF p_collection_id IS NOT NULL THEN
    ...
    FROM (
      SELECT u.id,
             public.url_effective_score(u.id, v_score_subcats, v_score_weights, v_recent_subcats) AS eff_score
...
-- Grant execute on the helper function too
REVOKE EXECUTE ON FUNCTION public.url_effective_score(UUID, UUID[], DOUBLE PRECISION[], UUID[]) FROM PUBLIC, anon;
GRANT  EXECUTE ON FUNCTION public.url_effective_score(UUID, UUID[], DOUBLE PRECISION[], UUID[]) TO authenticated;