...
--   1. roam_score_static column populated via backfill:
--        UPDATE public.urls
--          SET roam_score_static = wilson_score + 0.3 * seeder_score
--        WHERE roam_score_static IS NULL;
--   2. idx_urls_roam_score_static (roam_score_static DESC WHERE approved = TRUE)
--      exists and is built.
--
-- Changes from v16:
--   - Scoring expression: replace (u.wilson_score + 0.3 * u.seeder_score)
--     with u.roam_score_static in all four scoring blocks (collection Phase 1,
--     collection Phase 2, standard Phase 1, standard Phase 2).
--   - Phase 2 ORDER BY: replace (u.wilson_score + 0.3 * u.seeder_score) DESC
--     with u.roam_score_static DESC in both fallback subqueries, allowing
--     PostgreSQL to satisfy the ORDER BY via idx_urls_roam_score_static without
--     a runtime sort on the full candidate set.
--   - COALESCE fallback removed: column is fully populated; COALESCE is no
