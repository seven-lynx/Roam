...
--                   SemanticScholar:  citationCount / 1000
--                   all others:       0.0 (no signal available)
--
-- roam() v10 blends seeder_score into eff_score:
--   eff_score = (wilson_score + 0.3 * seeder_score) * calibrated_weight
-- For unrated URLs (wilson_score = 0) this lets high-signal content surface
...
ALTER TABLE public.urls
  ADD CONSTRAINT urls_seeder_score_range
...
CREATE INDEX IF NOT EXISTS idx_urls_seeder_score
...
BEGIN
  ...
  IF p_collection_id IS NOT NULL THEN
    ...
    FROM (
      SELECT u.id,
             -- v10: blend seeder_score for unrated URLs
