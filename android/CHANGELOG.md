...
### Added
- Daily challenges with XP rewards and real-time progress tracking. Complete tasks like rating URLs, discovering new pages, and exploring categories to earn bonus XP.
- Challenge screens (ChallengesScreen, ChallengeCard, ChallengeWidget) with progress bars and countdown timers.
- `user_actions` table and database trigger for real-time challenge progress updates.

### Changed
- Discovery algorithm v31–v35. Inline scoring eliminates N+1 per-row sub-queries, parallel batch RPC calls with `Promise.allSettled`, TABLESAMPLE increased from 5% to 15%, seen_ids cap reduced from 10K to 3K, Phase 2 LIMIT increased from 100 to 500, pool-empty guard with BERNOULLI(30) retry. Significantly faster response and fewer false exhaustion states.
- Android session wait reduced from 10s to 3s for faster queue refills.
- Roam health check endpoint added for CI gating and monitoring.

### Fixed
- Status bar no longer disappears during WebView URL loads.
- Submit-URL full-stack audit. Admin-moderation DB role lookup fixed, notification sent on approval, toast messages corrected, settings autofill improved.
- Badge text readability improved across all screens.
- Deno type errors resolved in evaluate-badges and challenge Edge Functions.
- `save-url` fire-and-forget chains replaced with async IIFE to prevent unhandled promise rejections.
- Hardcoded secrets removed from scripts directory.

### Backend
- Challenge system migration (`20260805000000_challenge_system.sql`) with daily challenge generation, progress tracking, and XP rewards.
- `user_actions` table and trigger for real-time challenge progress.
- `cron-daily-challenges` Edge Function for automated daily challenge seeding.
- `challenges` Edge Function for challenge queries and progress.
- `roam-health-check` Edge Function with GitHub Actions health-check workflow.
- Docker Hub login added to CI to fix rate limiting.
- Deploy workflow updated to include challenge functions.
