...
async function seedPubMed() {
  ...
  if (!progress.upsertComplete) {
    ...
    // Use upsertUrls with liveness check so dead pages are skipped
    const result = await upsertUrls(rows, { checkLive: true, fetchOg: false, verbose: true });
    const upsertedCount = result.inserted;
    ...
    console.log(`\n[pubmed] ✅ Upsert phase complete: ${upsertedCount} papers inserted (${result.skipped} skipped)`);
  } else {
