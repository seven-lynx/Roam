...
**Total: ~3.2M URLs across 37+ active sources** *(as of May 2026)*
...
| `_discoverHeadless(opts)` | **Tier 9 fallback** — headless Chromium crawl (Crawlee + Playwright) for JS-rendered/bot-protected sites. Only invoked when all fetch-based tiers yield <10 URLs. Dynamically imported — zero overhead when not needed. |
| `seedRssWithFallbacks(config)` | Multi-method seeder: RSS → Sitemap → Wayback CDX → RSS Autodiscovery → Headless |
| `seedWaybackCdx(config)` | Wayback Machine CDX seeder: CDX → Sitemap → RSS Autodiscovery → Headless |
...
Follow this checklist — it prevents FK violations and miscategorized content:

1. **Pick or create a subcategory:**
   - Check `scripts/lib/seed.js` for existing `SUBCATEGORY.*` constants.
   - If you need a new one, add it to `SUBCATEGORY` in `lib/seed.js` following the UUID scheme:
     `c2{pillar:06d}-0000-0000-0000-{sort_order:012d}`
   - Run `node scripts/ensure-subcategories.mjs --migrate` to auto-generate a migration.
   - Run `npx supabase db push` to apply it.

2. **Create the seeder file:**
   - `scripts/seed-{source}.mjs` (or `.js`)
   - Use `CATEGORY.X` and `SUBCATEGORY.Y` constants from `lib/seed.js` — never hardcode UUIDs.
   - Import `seedRssWithFallbacks` (for RSS/sitemap-based sources) or `upsertUrls` (for custom collection) from `lib/seed.js`.

3. **Validate categorization:**
   ```bash
   node scripts/validate-seeders.mjs
   ```
   This checks that every `category_id`/`subcategory_id` in your seeder actually exists in the DB and that the subcategory belongs to the right category. It exits non-zero on violations.

4. **Test the seeder:**
   - Dry-run: `node scripts/seed-{source}.mjs --dry-run` (if supported)
   - Full run: `node scripts/seed-{source}.mjs`
   - Verify: `node scripts/audit-seeders.mjs --health`

5. **Update documentation:**
   - Add a row to the Data Sources table above.
   - Add to `package.json` if you want it in batch runs.

### Management Tools

| Tool | Purpose |
|------|---------|
| `validate-seeders.mjs` | Pre-flight check: verifies all seeder category/subcategory IDs exist in DB. Run before any batch seeder operation. |
| `ensure-subcategories.mjs` | Sync `SUBCATEGORY` constants ↔ `subcategories` table. `--check` reports mismatches, `--migrate` generates a migration for missing rows. |
| `audit-seeders.mjs` | Health dashboard: color-coded per-seeder status, trends, stale detection. `--health`, `--worst N`, `--stale N`, `--report`. |
| `list-seeders.mjs` | Master list of all sources with cached (pending) and committed (DB) URL counts. |
| `log-seeding.mjs` | Local JSONL run logger (`scripts/.cache/seeding-runs.jsonl`). CLI: `--list`, `--stats`, `--seeder=X`. |
| `master-log.mjs` | Master log: shows every seeder's last run time, results, category, method, and status. `--all`, `--errors`, `--crashes`, `--seeder=X`, `--json`. |
| `flush-seeders.mjs` | 4-step pipeline: commit dead links → delete inactive → flush cached seeders → run master list. |

### Recommended Workflow

```bash
# 1. Validate everything before running
node scripts/validate-seeders.mjs

# 2. Check seeder health
node scripts/audit-seeders.mjs --health

# 3. Run individual or all seeders
node scripts/seed-{source}.mjs
# or flush all cached:
node scripts/flush-seeders.mjs --step flush

# 4. Review results
node scripts/audit-seeders.mjs --report
node scripts/log-seeding.mjs --stats
```
...
0 2 * * 0 cd /path/to/roam/scripts && node scripts/validate-seeders.mjs && node scripts/flush-seeders.mjs
```
