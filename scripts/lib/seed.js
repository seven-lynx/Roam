...
/**
 * Runs ALL discovery methods and merges results (deduplicated by URL).
 * Previously stopped at the first tier that returned ≥10 URLs;
 * now collects URLs from every possible source for maximum coverage.
 * Returns { entries, method } where entries is
 * [{ url, feedTitle, feedDesc, feedDate, timestamp? }] and method is
 * a combined string like "sitemap+wayback-cdx+headless".
 */
async function _discoverAll({ siteDomain, articlePathRegex, skipPaths, siteSuffixRegex, feedUrl, UA, maxArticles, maxPages }) {
  // Unified dedup map: key = normalized URL, value = entry + source methods
  const allMap = new Map();
  const sourceMethods = [];
  const seenMapUrls = new Set(); // for sitemap recursion, separate scope

  function _addEntries(entries, methodName) {
    let added = 0;
    for (const e of entries) {
      const norm = _normUrl(e.url) || e.url;
      if (!allMap.has(norm)) {
        allMap.set(norm, { ...e, url: norm });
        added++;
      }
    }
    if (added > 0) {
      sourceMethods.push(methodName);
      console.log(`  ✓ ${methodName}: ${entries.length} discovered, ${added} unique (total: ${allMap.size})`);
    } else if (entries.length > 0) {
      console.log(`  [${methodName}] ${entries.length} found, 0 unique (all duplicates)`);
    } else {
      console.log(`  [${methodName}] 0 URLs found`);
    }
  }
  ...
  if (feedUrl) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (text.length >= 50) {
          const seen = new Set();
          const entries = _parseFeedXml(text, feedUrl, articlePathRegex, skipPaths, siteSuffixRegex, maxArticles, seen);
          _addEntries(entries, "RSS");
        } else { console.log(`  [RSS] Empty response`); }
      } else { console.log(`  [RSS] HTTP ${res.status}`); }
    } catch (err) { console.log(`  [RSS] Error: ${err.message}`); }
  }

  // ── Tier 2: RSS from common paths ───────────────────────────────────────
  console.log(`\n📡 Tier 2: RSS common paths...`);
  const rssCommon = await _discoverRssCommonPaths({ siteDomain, articlePathRegex, skipPaths, siteSuffixRegex, UA, maxArticles });
  _addEntries(rssCommon, "RSS-common");

  // ── Tier 3: JSON Feed ───────────────────────────────────────────────────
  console.log(`\n📡 Tier 3: JSON Feed...`);
  const jsonFeed = await _discoverJsonFeed({ siteDomain, articlePathRegex, skipPaths, UA });
  _addEntries(jsonFeed, "JSON Feed");
  ...
  async function _parseSitemapAll(sitemapUrl) {
    if (seenMapUrls.has(sitemapUrl)) return;
    seenMapUrls.add(sitemapUrl);
    try {
      ...
      if (/<sitemapindex/i.test(text)) {
        for (const m of text.matchAll(/<loc>([^<]+)<\/loc>/gi)) {
          const url = m[1].trim();
          if (url.startsWith("http") && !seenMapUrls.has(url)) { await _parseSitemapAll(url); }
        }
        return;
      }
      for (const m of text.matchAll(/<url>\s*<loc>([^<]+)<\/loc>/gi)) {
        const link = _normUrl(m[1].trim());
        if (link && _filterUrl(link, articlePathRegex, skipPaths, seenMapUrls)) {
          seenMapUrls.add(link);
          sitemapEntries.push({ url: link, feedTitle: null, feedDesc: null, feedDate: null });
          if (sitemapEntries.length >= maxArticles) return;
        }
      }
    } catch { /* skip */ }
  }
  ...
  for (const sitemapUrl of allSitemapUrls) {
    if (sitemapEntries.length >= maxArticles) break;
    await _parseSitemapAll(sitemapUrl);
  }
  _addEntries(sitemapEntries, "sitemap");

  // ── Tier 5: WordPress REST API ──────────────────────────────────────────
  console.log(`\n📡 Tier 5: WordPress REST API...`);
  const wpEntries = await _discoverWpApi({ siteDomain, articlePathRegex, skipPaths, UA });
  _addEntries(wpEntries, "WP API");

  // (Tier 6: Wayback CDX — handled by seedWaybackCdx separately, skip here)

  // ── Tier 7: Homepage RSS autodiscovery ──────────────────────────────────
  console.log(`\n📡 Tier 7: Homepage RSS autodiscovery...`);
  const rssAuto = await _discoverRssAuto({ siteDomain, articlePathRegex, skipPaths, siteSuffixRegex, UA, maxArticles });
  _addEntries(rssAuto, "RSS-auto");

  // ── Tier 8: Archive page scraping ───────────────────────────────────────
  console.log(`\n📡 Tier 8: Archive page scraping...`);
  const archiveEntries = await _discoverArchivePages({ siteDomain, articlePathRegex, skipPaths, UA });
  _addEntries(archiveEntries, "archive-scrape");

  // ── Tier 9: Headless browser (Crawlee + Playwright) ────────────────────
  ...
  _addEntries(headlessEntries, "headless");

  // ── Build result ────────────────────────────────────────────────────────
  const entries = [...allMap.values()];
  const method = sourceMethods.length > 0 ? sourceMethods.join("+") : "none";
  console.log(`\n📊 Discovery complete: ${entries.length} unique URLs from ${sourceMethods.length} methods [${method}]`);
...
export async function seedRssWithFallbacks(config) {
  ...
  let discovered = cache.discovered;
  let method = "cached";
  const isWaybackEntry = {}; // track which entries came from Wayback (function scope — used after discovery block)

  if (!discovered.length || NO_CACHE) {
    // Collect URLs from ALL methods, then deduplicate
    const allMap = new Map();
    const sourceMethods = [];

    function _addEntries(entries, methodName, isWb = false) {
      let added = 0;
      for (const e of entries) {
        const norm = _normUrl(e.url) || e.url;
        if (!allMap.has(norm)) {
          allMap.set(norm, { ...e, url: norm });
          if (isWb) isWaybackEntry[norm] = true;
          added++;
        }
      }
      if (added > 0) {
    ...
    const rssEntries = await discoverUrlsFromRss();
    _addEntries(rssEntries, "RSS");

    // Tier 2: Sitemap
    console.log(`\n📡 Tier 2: Sitemap discovery...`);
    const sitemapEntries = await discoverUrlsFromSitemap();
    _addEntries(sitemapEntries, "sitemap");

    // Tier 3: Wayback CDX
    console.log(`\n📡 Tier 3: Wayback CDX...`);
    const wbResults = await discoverUrlsFromWayback();
    const wbEntries = wbResults.map(r => ({ url: r.url, feedTitle: null, feedDesc: null, feedDate: null, timestamp: r.timestamp }));
    _addEntries(wbEntries, "Wayback", true);

    // Tier 4: RSS autodiscovery from homepage
    console.log(`\n📡 Tier 4: RSS autodiscovery from homepage...`);
    const rssAutoResults = await discoverUrlsFromRssAutodiscovery();
    _addEntries(rssAutoResults.map(r => ({ url: r.url, feedTitle: null, feedDesc: null, feedDate: null, timestamp: null })), "RSS-autodiscovery");

    // Tier 5: Headless browser
    console.log(`\n📡 Tier 5: Headless browser crawl...`);
    const headlessEntries = await _discoverHeadless({ siteDomain, articlePathRegex, skipPaths, siteSuffixRegex, UA, maxArticles });
    _addEntries(headlessEntries.map(r => ({ url: r.url, feedTitle: r.feedTitle, feedDesc: r.feedDesc, feedDate: r.feedDate, timestamp: null })), "headless");

    // Build final discovered array
    discovered = [...allMap.values()];
    method = sourceMethods.length > 0 ? sourceMethods.join("+") : "none";
    console.log(`\n📊 Discovery complete: ${discovered.length} unique URLs from ${sourceMethods.length} methods [${method}]`);
  ...
  // Determine if this seeder used any Wayback CDX entries (they won't resolve live)
  const isWayback = discovered.some(e => isWaybackEntry[e.url]);
