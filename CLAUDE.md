...
Before any significant task, read `docs/CONTEXT.md` (gitignored) for current state and blockers.
...
**Web routes:** `/` landing, `/join` onboarding, `/u/[username]` profile, `/c/[slug]` collections, `/submit`, `/admin`, `/profile`, `/settings`, `/privacy`, `/terms`, `/how-it-works`, `/beta`, `/forgot-password`.

**Edge Functions:** `roam`, `rate`, `submit-url`, `profile`, `collection`, `follow`, `save-url`, `feedback`, `report-url`, `log-failed-urls`, `export-user`, `delete-user`, `beta-signup`, `send-bulk-email`.
...
- Queue state lives in `chrome.storage.local`. Never in-memory.
...
- RPC contracts in `supabase/API.md`.
