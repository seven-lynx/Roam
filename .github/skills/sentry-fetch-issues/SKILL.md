---
name: sentry-fetch-issues
description: "Fetch and diagnose Sentry issues via REST API. Use when: investigating crash reports, fetching event details, parsing Sentry JSON manually (PowerShell ConvertFrom-Json fails on deep Sentry payloads). Prefer this over sentry-cli for richer issue detail."
argument-hint: "<command>: list unresolved, fetch event by ID, parse event, resolve issue"
---
...
# Org/Project slug — see copilot-instructions.md Sentry section
Invoke-RestMethod "https://us.sentry.io/api/0/projects/<ORG_SLUG>/<PROJECT_SLUG>/issues/?query=is:unresolved&limit=25" -Headers $headers |
...
```powershell
$r = Invoke-WebRequest "https://us.sentry.io/api/0/issues/<NUMERIC_ID>/events/latest/" -Headers $headers -UseBasicParsing
...
Invoke-RestMethod "https://us.sentry.io/api/0/issues/<NUMERIC_ID>/" -Method Put `
  -Headers $headers -Body '{"status":"resolved"}' -ContentType "application/json"
```
