...
export default async function Home() {
  ...
  function fmtCount(n: number): string {
    if (n >= 1000000) { const m = (n / 1000000).toFixed(1); return `${m} million`; }
    if (n >= 1000) { const k = (n / 1000).toFixed(1); return `${k}K`; }
    return `${n}`;
  }
          ...
          <div className="flex flex-col lg:flex-row items-center gap-12">
            <div className="flex-1 text-center lg:text-left">
              ...
              <div className="flex items-center gap-6 mt-10 justify-center lg:justify-start">
                <div className="text-center"><div className="text-2xl font-bold text-zinc-900 dark:text-white">1.6 million</div><div className="text-xs text-zinc-400 mt-0.5">Curated URLs</div></div>
                <div className="w-px h-10 bg-zinc-200 dark:bg-zinc-800" />
                <div className="text-center"><div className="text-2xl font-bold text-zinc-900 dark:text-white">72+</div><div className="text-xs text-zinc-400 mt-0.5">Topics</div></div>
                <div className="w-px h-10 bg-zinc-200 dark:bg-zinc-800" />
                <div className="text-center"><div className="text-2xl font-bold text-zinc-900 dark:text-white">{ratingCount !== null ? fmtCount(ratingCount) : '4.2 million'}</div><div className="text-xs text-zinc-400 mt-0.5">Total page ratings</div></div>
              </div>
            </div>
            <div className="w-full max-w-md lg:max-w-lg">
              <Suspense fallback={<div className="animate-pulse rounded-2xl border border-zinc-200 dark:border-zinc-800 p-6"><div className="aspect-video bg-zinc-200 dark:bg-zinc-800 rounded-xl mb-4" /><div className="h-5 w-3/4 bg-zinc-200 dark:bg-zinc-800 rounded mb-2" /><div className="h-4 w-1/3 bg-zinc-200 dark:bg-zinc-800 rounded" /></div>}><SiteShowcase /></Suspense>
            </div>
          </div>
        </div>
      </section>
      ...
      {user && (
        <section className="max-w-6xl mx-auto px-6 pb-20">
          <div className="rounded-2xl border border-amber-200 dark:border-amber-800/40 bg-gradient-to-br from-amber-50 to-amber-100/50 dark:from-amber-900/10 dark:to-amber-950/20 p-8 md:p-10 flex flex-col md:flex-row items-center gap-6">
            <div className="flex-1 text-center md:text-left"><h2 className="text-2xl font-bold text-zinc-900 dark:text-white mb-2">Found something worth sharing?</h2><p className="text-zinc-500 dark:text-zinc-400 max-w-md">Submit a page to our community-curated catalog. Real people review every submission to keep quality high.</p></div>
            <Link href="/submit" className="shrink-0 inline-flex items-center gap-2 rounded-full bg-amber-500 hover:bg-amber-400 px-8 py-3 text-white font-semibold text-base transition-colors"><span className="text-lg">+</span> Submit a URL</Link>
          </div>
        </section>
      )}
      <section className="max-w-6xl mx-auto px-6 pb-20">
      ...
      {!user && (
        <section className="max-w-6xl mx-auto px-6 pb-20 text-center">
          <div className="rounded-2xl border border-amber-200 dark:border-amber-800/40 bg-amber-50/50 dark:bg-amber-900/10 p-10">
            <h2 className="text-2xl font-bold text-zinc-900 dark:text-white mb-3">Ready to explore?</h2><p className="text-zinc-500 dark:text-zinc-400 mb-6 max-w-md mx-auto">Sign up to get personalised recommendations, or try a random curated page right now.</p>
            <div className="flex flex-col sm:flex-row gap-3 justify-center"><Link href="/signup" className="inline-flex items-center justify-center rounded-full bg-amber-500 hover:bg-amber-400 px-8 py-3 text-white font-semibold text-base transition-colors">Get started</Link><RandomPageButton /></div>
          </div>
        </section>
      )}
...
function TopSitesSkeleton() {
  return <div className="w-full pt-10 animate-pulse"><div className="h-7 w-40 bg-zinc-200 dark:bg-zinc-800 rounded mb-6" /><div className="grid gap-3">{Array.from({ length: 5 }).map((_, i) => (<div key={i} className="flex items-center gap-4 rounded-xl border border-zinc-200 dark:border-zinc-800 px-4 py-3"><div className="w-5 h-5 rounded bg-zinc-200 dark:bg-zinc-800 shrink-0" /><div className="flex-1 min-w-0"><div className="h-4 w-3/4 bg-zinc-200 dark:bg-zinc-800 rounded mb-1" /><div className="h-3 w-1/3 bg-zinc-200 dark:bg-zinc-800 rounded" /></div><div className="h-4 w-8 bg-zinc-200 dark:bg-zinc-800 rounded shrink-0" /></div>))}</div></div>;
}

async function FeaturedCollections() {
  let supabase;
  try { supabase = await createClient(); } catch { return null; }
  const { data: collections } = await supabase.from('collections').select('id, name, slug, profiles(username, display_name)').eq('is_public', true).order('created_at', { ascending: false }).limit(6);
