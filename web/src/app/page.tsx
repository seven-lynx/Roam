          ...
          <div className="flex flex-col lg:flex-row items-center gap-12">
            <div className="flex-1 text-center lg:text-left">
              ...
              <div className="flex flex-col sm:flex-row gap-3 mt-8 justify-center lg:justify-start">
                {user ? (
                  <>
                    <Link href="/profile" className="inline-flex items-center justify-center rounded-full bg-amber-500 hover:bg-amber-400 px-8 py-3 text-white font-semibold text-base transition-colors">
                      Go to profile
                    </Link>
                    <Link href="/how-it-works" className="inline-flex items-center justify-center rounded-full border border-zinc-300 dark:border-zinc-700 px-8 py-3 text-zinc-800 dark:text-zinc-200 font-semibold text-base hover:bg-zinc-50 dark:hover:bg-zinc-900 transition-colors">
                      How it works →
                    </Link>
                  </>
                ) : (
                  <>
                    <Link href="/signup" className="inline-flex items-center justify-center rounded-full bg-amber-500 hover:bg-amber-400 text-white px-8 py-3 font-semibold text-base transition-colors">
                      Get started
                    </Link>
                    <a href="#downloads" className="inline-flex items-center justify-center rounded-full border border-zinc-300 dark:border-zinc-700 px-8 py-3 text-zinc-800 dark:text-zinc-200 font-semibold text-base hover:bg-zinc-50 dark:hover:bg-zinc-900 transition-colors">
                      Get the extension ↓
                    </a>
                  </>
                )}
              ...
              <div className="flex items-center gap-6 mt-10 justify-center lg:justify-start">
                <div className="text-center">
                  <div className="text-2xl font-bold text-zinc-900 dark:text-white">1.6 million</div>
        ...
        <section className="max-w-6xl mx-auto px-6 pb-20">
          <div className="rounded-2xl border border-amber-200 dark:border-amber-800/40 bg-gradient-to-br from-amber-50 to-amber-100/50 dark:from-amber-900/10 dark:to-amber-950/20 p-8 md:p-10 flex flex-col md:flex-row items-center gap-6">
            <div className="flex-1 text-center md:text-left">
              <h2 className="text-2xl font-bold text-zinc-900 dark:text-white mb-2">Found something worth sharing?</h2>
              <p className="text-zinc-500 dark:text-zinc-400 max-w-md">Submit a page to our community-curated catalog. Real people review every submission to keep quality high.</p>
            </div>
            <Link href="/submit" className="shrink-0 inline-flex items-center gap-2 rounded-full bg-amber-500 hover:bg-amber-400 px-8 py-3 text-white font-semibold text-base transition-colors">
              <span className="text-lg">+</span> Submit a URL
            </Link>
          </div>
        </section>
      )}
      ...
      {!user && (
        <section className="max-w-6xl mx-auto px-6 pb-20 text-center">
          <div className="rounded-2xl border border-amber-200 dark:border-amber-800/40 bg-amber-50/50 dark:bg-amber-900/10 p-10">
            <h2 className="text-2xl font-bold text-zinc-900 dark:text-white mb-3">Ready to explore?</h2>
            <p className="text-zinc-500 dark:text-zinc-400 mb-6 max-w-md mx-auto">Sign up to get personalised recommendations, or try a random curated page right now.</p>
            <div className="flex flex-col sm:flex-row gap-3 justify-center">
              <Link href="/signup" className="inline-flex items-center justify-center rounded-full bg-amber-500 hover:bg-amber-400 px-8 py-3 text-white font-semibold text-base transition-colors">Get started</Link>
              <RandomPageButton />
            </div>
          </div>
        </section>
          ...
          <div className="rounded-2xl border border-zinc-200 dark:border-zinc-800 p-8 card-hover">
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-zinc-500 dark:text-zinc-400 mt-1 leading-relaxed">Click the Roam button while browsing to discover new pages. Rate with 👍👎 to personalize your recommendations.</p>
                <div className="flex gap-3 mt-4">
                  <a href="https://chromewebstore.google.com/detail/ojgphkdgkefokhjnojkddhalnlbajfpc?utm_source=roam-web" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 rounded-lg bg-zinc-900 dark:bg-white text-white dark:text-zinc-900 px-4 py-2 text-sm font-medium hover:opacity-90 transition-opacity">Chrome</a>
                  <a href="https://addons.mozilla.org/firefox/addon/roam-the-web/" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 rounded-lg border border-zinc-300 dark:border-zinc-700 px-4 py-2 text-sm font-medium text-zinc-700 dark:text-zinc-300 hover:bg-zinc-50 dark:hover:bg-zinc-900 transition-colors">Firefox</a>
                </div>
              </div>
            </div>
          </div>
          <div className="rounded-2xl border border-zinc-200 dark:border-zinc-800 p-8 card-hover">
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-zinc-500 dark:text-zinc-400 mt-1 leading-relaxed">Swipe to discover. Tap to save. Read offline. The full Roam experience in your pocket.</p>
                <div className="mt-4">
                  <a href="https://play.google.com/store/apps/details?id=app.roam.android" target="_blank" rel="noopener noreferrer" className="inline-flex items-center rounded-lg bg-zinc-900 dark:bg-white text-white dark:text-zinc-900 px-4 py-2 text-sm font-medium hover:opacity-90 transition-opacity">Download on Google Play</a>
                </div>
              </div>
            </div>
          </div>
    ...
    <div className="w-full pt-10 animate-pulse">
      ...
      <div className="grid gap-3">
        {Array.from({ length: 5 }).map((_, i) => (
          <div key={i} className="flex items-center gap-4 rounded-xl border border-zinc-200 dark:border-zinc-800 px-4 py-3">
...
async function TopExplorers() {
  ...
  const { data: explorers } = await supabase
    .from('profiles')
    .select('username, display_name, xp_total, level, badge_count, streak_days')
    .eq('is_public', true)
    .gt('xp_total', 0)
  ...
  return (
    <section className="max-w-6xl mx-auto px-6 pb-20">
      <div className="flex items-center justify-between mb-8">
        <h2 className="text-3xl font-bold text-zinc-900 dark:text-white">Top Explorers</h2>
        <Link href="/leaderboard" className="text-sm font-medium text-amber-600 dark:text-amber-400 hover:underline">
          Full leaderboard →
        </Link>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {explorers.map((explorer) => (
          <Link
            ...
            href={`/u/${explorer.username}`}
            className="flex items-center gap-4 rounded-xl border border-zinc-200 dark:border-zinc-800 p-4 hover:bg-zinc-50 dark:hover:bg-zinc-900 transition-colors group"
          >
            <div className="flex items-center justify-center w-10 h-10 rounded-full bg-gradient-to-br from-purple-500 to-blue-600 text-white font-bold text-sm shrink-0">
              {explorer.level}
            </div>
            <div className="flex-1 min-w-0">
              <p className="font-semibold text-sm text-zinc-900 dark:text-white truncate group-hover:underline">
                {explorer.display_name || explorer.username}
              </p>
              <p className="text-xs text-zinc-500 dark:text-zinc-400">Lv.{explorer.level} · {explorer.xp_total.toLocaleString()} XP</p>
              <div className="flex items-center gap-3 text-xs text-zinc-400 mt-0.5">
                <span>🏅 {explorer.badge_count}</span>
                {explorer.streak_days > 0 && <span>🔥 {explorer.streak_days}</span>}
              </div>
            </div>
          </Link>
        ))}
  ...
  return (
    <section className="max-w-6xl mx-auto px-6 pb-20">
      <div className="flex items-center justify-between mb-8">
        ...
        <Link href="/collections" className="text-sm font-medium text-amber-600 dark:text-amber-400 hover:underline">View all →</Link>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {collections.map((col) => {
          const owner = col.profiles as unknown as { username: string; display_name: string } | null;
          return (
            <Link key={col.id} href={`/collections/${col.slug}`} className="flex flex-col justify-between rounded-xl border border-zinc-200 dark:border-zinc-800 p-5 hover:bg-zinc-50 dark:hover:bg-zinc-900 transition-colors group">
              <div>
                <h3 className="text-base font-semibold text-zinc-900 dark:text-white group-hover:underline">{col.name}</h3>
                {owner && <p className="text-xs text-zinc-500 dark:text-zinc-400 mt-1">by {owner.display_name || owner.username}</p>}
              </div>
            </Link>
          );
        })}
