...
import { grantBadgeAdmin, revokeBadgeAdmin } from '@/app/admin/actions';
...
export function AdminBadges() {
  ...
  const [giftView, setGiftView] = useState<"grant" | "revoke" | "all">("all");
  ...
  async function revokeBadge() {
    ...
    setGiftError(null);
    try {
      const { data, error } = await revokeBadgeAdmin(giftUsername, giftBadgeSlug);
      if (error) throw new Error(error);
      setGiftResult(data?.message ?? `Badge "${giftBadgeSlug}" revoked from @${giftUsername}!`);
      setGiftUsername('');
      setGiftBadgeSlug('');
      loadBadges();
    } catch (e) {
      setGiftError(e instanceof Error ? e.message : 'Failed to revoke badge');
    }
    setGiftLoading(false);
  }
    ...
    <div className="flex flex-col gap-6">
      ...
      {giftView === "grant" && (
        <div className="rounded-xl border border-zinc-200 dark:border-zinc-800 p-6">
          <h2 className="text-lg font-semibold text-zinc-900 dark:text-white mb-4">Grant a Badge</h2>
      ...
      {/* Revoke Mode */}
      {giftView === "revoke" && (
        <div className="rounded-xl border border-red-200 dark:border-red-800 p-6">
          <h2 className="text-lg font-semibold text-zinc-900 dark:text-white mb-1">Revoke a Badge</h2>
          <p className="text-xs text-zinc-400 mb-4">Removes the badge so the user can earn it legitimately. XP is deducted.</p>
